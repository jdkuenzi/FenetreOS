...
{
	return syscall(SYSCALL_SET_CUSRSOR, x, y + 1, 0, 0);
}

int enable_cursor()
{
	return syscall(SYSCALL_ENABLE_CURSOR, 0, 0, 0, 0);
}

int disable_cursor()
{
	return syscall(SYSCALL_DISABLE_CURSOR, 0, 0, 0, 0);
}

int clean_vid_x_y(uint32_t x, uint32_t y)
{
	return syscall(SYSCALL_CLEAN_X_Y, x, y + 1, 0, 0);
}

int puts_x_y(char *s, uint32_t x, uint32_t y)
{
	return syscall(SYSCALL_PUTS_X_Y, (uint32_t)s, x, y + 1, 0);
}
...
int ls()
{
	return syscall(SYSCALL_LS, 0, 0, 0, 0);
}
